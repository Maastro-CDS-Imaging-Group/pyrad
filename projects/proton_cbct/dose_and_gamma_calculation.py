...
def calculate_dose(patient):
    pyrad_dose_calculation = dose_calculation.DoseCalculation(config="./projects/proton_cbct/configs/plan_config.yaml", projects_dir="./projects/proton_cbct")
    CT = patient / "deformed.nrrd"
    CBCT = patient / "target.nrrd"
    # sCT = patient / "translated.nrrd"
    body_mask = patient / "BODY.nrrd"

    if not (CT.exists() and CBCT.exists() and body_mask.exists()):
